// Based on https://github.com/inrupt/websockets-pubsub/blob/master/src/lib/hub.ts
...
export class Hub {
  ...
  async handleConnection(
    ...
    ws.on("message", function incoming(message: string): void {
      ...
      if (
        ...
        SUBSCRIBE_COMMAND_PREFIX
      ) {
        newClient.subscriptions.push(
          new URL(message.substring(SUBSCRIBE_COMMAND_PREFIX.length))
        );
  ...
  async getWebIdFromAuthorizationHeader(
    ...
    const { webId } = await determineWebIdAndOrigin(
  ...
  async getWebIdFromQueryParameter(
    url: URL,
    origin: string
  ): Promise<URL | undefined> {
  ...
  async getWebId(httpReq: http.IncomingMessage): Promise<URL | undefined> {
    ...
    const fromAuthorizationHeader = await this.getWebIdFromAuthorizationHeader(
      httpReq.headers,
      origin || ""
    );
    ...
    if (httpReq.url) {
      debug(
        ...
        httpReq.url,
        this.audience,
        new URL(httpReq.url, this.audience)
      );
      return this.getWebIdFromQueryParameter(
        new URL(httpReq.url, this.audience),
        origin || ""
      );
    }
    ...
    this.clients.map(async client => {
      ...
      client.subscriptions.map(async subscription => {
        debug(
          ...
          hasPrefix(url.toString(), subscription.toString())
        );
        ...
        const hasAccess = await this.wacLdp.hasAccess(
          webId,
          client.origin,
          url,
          ACL.Read
